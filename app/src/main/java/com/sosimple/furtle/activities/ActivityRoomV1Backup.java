...
public class ActivityRoomV1Backup extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //TODO:manuel signup ve signin yerine google sign in gelecek,
        // o geldikten sonra google silent signin sonucu gelen token ile backend tarafında sigin yapılacak
        // backend signinden gelen token strtoken degiskenine yazilacak
        ...
        playPauseButton.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                //TODO:REMOVE THESE CODES AFTER BACKEND TEST
//                testSignUp();
                //TODO:post signin method will be google silent signin method
                postSignIn();
//                getSignupTokenFromFromJSON("{\"ok\":true,\"token\":\"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6Im11aGFtbWV0dW11dGFrc295QGdtYWlsLmNvbSIsInVzZXJuYW1lIjoidW11dGFrc295MTIzNDUiLCJwcCI6bnVsbCwiaXAiOiIxMC4wLjEuNCIsImlhdCI6MTU5NTA3NDc0M30.DJeNnVGm7Q66RA2ggE2-AX31jaIm0lXp3KTeY8nmtdE\",\"user\":{\"email\":\"muhammetumutaksoy@gmail.com\",\"username\":\"umutaksoy12345\"}}");
                //TODO:REMOVE THESE CODES AFTER BACKEND TEST
        ...
        if (bIsUserLoggedIn){
            //TODO:use joinroom method instead of getroomactualdata method
...
//        showInterstitialAd();
    ...
    /*NO NEEDED, BECAUSE STARTING TO USE SOCKET IO
...
//                new Handler().postDelayed(this::postSigninOrSignup, 200);
                new Handler().postDelayed(this::postSignup, 200);
*/
            }else{
...
//        signInPanel.setAlpha(0.4f);
    ...
    private void postSignIn() {
        ...
        try {
            paramObject.put("email", "muhammet@gmail.com");
            paramObject.put("password", "m123456*");
        ...
        Log.e("umutsignup", "postSignup body json : " + paramObject.toString());
        this.callbackCall = RestAdapter.createAPI().signIn(paramObject.toString());
        this.callbackCall.enqueue(new Callback<CallBackSignup>() {
            public void onResponse(Call<CallBackSignup> call, Response<CallBackSignup> response) {
                ...
                String strResponseErrorBody = null;
                strBackendToken = null;
                ...
                try {
                    ...
                    strBackendToken = null;
                    Tools.processLogs(2, "kullanici kayit isleminde response body alinamadi, detay : " + e.toString());
                }

                try {
                    assert response.errorBody() != null;
                    strResponseErrorBody = response.errorBody().toString();
                    Log.e("umutsocket", "signin onResponse erro body : " + strResponseErrorBody);
                }catch (Exception e){
                    Tools.processLogs(2, "kullanici kayit isleminde response error body alinamadi, detay : " + e.toString());
                }

                //operation is successful, get the token from response json
                if (iResponseCode == 201 && strBackendToken != null){
                    Tools.processLogs(0, "kullanici kayit basarili, kullanici : " + myUser.userEmail);

                    Tools.processLogs(2, "response token : " + strBackendToken);
                    bJoinNeeded = true;
                    bFocusIsNeeded = true;
                    initSocket();
                    ...
                    Tools.processLogs(2, "kullanici kayit hatasi, response sonucu, kod : " + iResponseCode);

                    if (strResponseErrorBody != null){
                        Tools.processLogs(2, "kullanici kayit hatasi, response error body : " + strResponseErrorBody);
                    }
            ...
            public void onFailure(Call<CallBackSignup> call, Throwable th) {
                th.printStackTrace();
                Log.e("umutsignup2","signup api onFailure " +  th.getMessage());
                if (!call.isCanceled()) {
                    onFailRequest();
                }
            }
    ...
    private void postGoogleSignIn(){
        //TODO: postSignUp ve postSignIn metodlarını karşılayan google sign in metodu yazılacak.
        // bunun için furtle'ın orjinal google developer console bilgileri gerekiyor
        ...
        this.callbackCall.enqueue(new Callback<CallBackSignup>() {
            public void onResponse(Call<CallBackSignup> call, Response<CallBackSignup> response) {
                ...
                try {
                    assert response.body() != null;
                    Log.e("umutgoogle", "google signin onResponse body : " + response.body().token.toString());
                } catch (Exception e) {
                    Tools.processLogs(2, "kullanici kayit isleminde response body alinamadi, detay : " + e.toString());
                }
...
//                initSocket();
    ...
    private void exitRoom(){
        ...
        Log.e("umutsocket", "socketio exitRoom calisiyor");

        if (mSocket != null){
            Log.e("umutsocket", "socketio exitRoom socket nulldan farkli");

            if (mSocket.connected()){
                Log.e("umutsocket", "socketio exitRoom socket bagli, cikis yap komutu gonderiliyor");
                mSocket.off(strSocketJoin);
            }

        }else{
            Log.e("umutsocket", "socketio exitRoom socket null geldi");
        }
...
//            initSocket();
    ...
    private void onSuccessRequest(){
        //TODO:burada kullanici basariyla kayit oldu ve giris yapti. kullanicinin odaya girdiginin backend tarafina iletilmesi lazim
        Log.e("umutsignup", "onSuccessRequest calisti");
    }

    private String getUserDataFromRoomJSON(String strJSON){
        //TODO:room data json formati = {"email":"muhammetumutaksoy@gmail.com","pp":"https:\/\/cdn.sosimple.tech\/profile_photos\/default.png"}
...
//            strEmail = mainJson.getJSONObject(0).getString("email");

            for (int i = 0; i < mainJson.length(); i++){
                strEmail = mainJson.getJSONObject(i).getString("email");
        ...
        public void call(final Object... args) {
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        String strRoomEmail = getUserDataFromRoomJSON(args[0].toString());
                        Log.e("umutsocket", "roomInsidersChanged room data first email : " + strRoomEmail);
                    }catch (Exception e){
